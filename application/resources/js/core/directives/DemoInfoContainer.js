...
.directive('demoInfoContainer', ['$compile', 'localStorage', 'users', 'utils', function($compile, localStorage, users, utils) {
    return {
        ...
        link: function($scope, el) {
            var creds = localStorage.get('picturish-demo-credentials');
            ...
            var html = '<div>'+
                ...
                '<ol>'+
                    '<li>There is no general demo account. Click the button below or register to create one.</li>'+
                    '<li>On Demo site admin area is accessible to everyone.</li>'+
                    '<li>Demo accounts will be reset every 2 days. (photos, activity etc will be deleted).</li>'+
                    '<li>Accounts on demo site are limited to 40MB of storage space.</li>'+
                    '<li>Demo accounts created via the button below will be populated with sample images.</li>'+
                '</ol>'+
            ...
            el.find('.md-button').on('click', function() {
                ...
                users.register(credentials).success(function(data) {
                    localStorage.set('picturish-demo-credentials', { email: data.email, password: 'demo' });
